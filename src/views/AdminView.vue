            ...
            class="h-full max-h-full w-full flex flex-col bg-neutral-900/70 backdrop-blur-md rounded-xl overflow-hidden">
              ...
              <div class="relative flex-1 flex flex-col text-neutral-400 overflow-auto">
                  ...
                  <table class="w-full min-w-max">
                      ...
                      <tbody  v-if="arrAdmin != null">
                        <tr v-for="(x,i) in arrAdmin" :key="x">
...
export default {
  ...
  setup() {
    ...
    // vuex state
    let arrAdmin = computed(() => {
      return store.state.adminapi.admin;
    });

    // search
    const doSearch = (event) => {
      let key      = event.target.value;
      let tmpAdmin = store.state.adminapi.tmpAdmin;
      let admin    = tmpAdmin.filter(e => {
        if (e.username.includes(key)) {
          return e;
        }
      })

      store.commit("adminapi/SET_ADMIN",admin);
    }
    ...
    const showForm = (event,data = {}) => {
      ...
      store.commit("formAddEditAdminTeacherApi/SET_PRIVILEGE",'admin');
    }

    // delete admin
    const deleteAdmin = (event,id) => {
        Swal.fire({
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                store.commit("adminapi/SET_ADMIN",admin);
                store.commit("adminapi/SET_TMPADMIN",admin);
                store.dispatch("adminapi/DELETE_ADMIN",id);
            } 
    ...
    onMounted(() => {
      store.dispatch("adminapi/GET_ADMIN");
    })
